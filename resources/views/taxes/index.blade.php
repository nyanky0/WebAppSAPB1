        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3 shrink-0" x-data="{ isSyncing: false }">
                ...
                <form action="{{ route('taxes.sync') }}" method="POST" @submit="isSyncing = true">
                    @csrf
                    <button type="submit"
                        class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 transition-colors">
                ...
                <div x-show="isSyncing" class="fixed inset-0 z-[200] overflow-y-auto" style="display: none;">
                    <div
                        ...
                        <div
                            ...
                            <p class="text-sm text-gray-500">Please wait while we fetch the latest Tax Codes from SAP
                                Service Layer.</p>
                            <p class="text-xs text-indigo-600 mt-2 font-medium">Auto-retry enabled (up to
                                {{ \App\Models\Config::first()?->max_retries ?? 3 }} attempts on network/server errors)
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <form method="GET" action="{{ route('taxes.index') }}" class="flex flex-1 flex-wrap gap-4 items-center">
                ...
                <div>
                    <select name="status" onchange="this.form.submit()"
                        class="rounded-md border-0 py-2 pl-3 pr-8 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                        <option value="active" {{ request('status', 'active') === 'active' ? 'selected' : '' }}>Active
                            Taxes</option>
                        <option value="locked" {{ request('status') === 'locked' ? 'selected' : '' }}>Locked Taxes
                        </option>
                ...
                <div>
                    <select name="per_page" onchange="this.form.submit()"
                        class="rounded-md border-0 py-2 pl-3 pr-8 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                        <option value="20" {{ request('per_page', 20) == 20 ? 'selected' : '' }}>20 per page
                        </option>
                ...
                <button type="submit"
                    class="rounded-md bg-white px-3.5 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 transition-colors">Search</button>
                @if (request()->filled('search') || request()->filled('status'))
                    <a href="{{ route('taxes.index') }}"
                        class="rounded-md bg-white px-3.5 py-2 text-sm font-semibold text-red-600 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 transition-colors">Clear
                        Filter</a>
                @endif
            </form>
        </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-300">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col"
                                        class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Rate (%)</th>
                                    <th scope="col"
                                ...
                                @forelse($taxes as $tax)
                                    <tr>
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            {{ $tax->name }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            {{ number_format($tax->rate, 2) }}%</td>

                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            @if ($tax->locked)
                                                <span
                                                    class="inline-flex items-center rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10">Locked</span>
                                            @else
                                                <span
