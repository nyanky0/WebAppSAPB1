<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Dimensions Master Data</h1>
                <p class="mt-1 text-sm text-gray-500">A local synchronized list of SAP Business One Costing Dimensions (e.g. Department, Project, Branch).</p>
            </div>
            <div class="flex items-center gap-3 shrink-0" x-data="{ isSyncing: false }">
                <form action="{{ route('dimensions.sync') }}" method="POST" @submit="isSyncing = true">
                    @csrf
                ...
                <div x-show="isSyncing" class="fixed inset-0 z-[200] overflow-y-auto" style="display: none;">
                    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div class="relative inline-block align-middle bg-white/90 backdrop-blur-xl border border-white/40 rounded-xl text-left overflow-hidden shadow-2xl transform sm:my-8 sm:max-w-md sm:w-full p-8 text-center">
                            ...
                            <h3 class="text-xl font-semibold text-gray-900 mb-2">Syncing Dimensions...</h3>
                            <p class="text-sm text-gray-500">Please wait while we fetch Dimensions from SAP Service Layer.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Filter / Search -->
        <div class="mt-8">
            <form method="GET" action="{{ route('dimensions.index') }}" class="flex flex-wrap items-center gap-4">
                ...
                @if(request()->has('search') || request()->has('per_page'))
                    <a href="{{ route('dimensions.index') }}" class="rounded-md bg-white px-3 py-2 text-sm font-semibold text-red-600 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">Clear</a>
                @endif
            </form>
        </div>
                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg bg-white">
                        <table class="min-w-full divide-y divide-gray-300">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Dimension Code</th>
                                    <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Dimension Name</th>
                                    <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-gray-900">Status</th>
                                    <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-gray-900">Sync Status</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 bg-white">
                                @forelse($dimensions as $dim)
                                    <tr class="hover:bg-gray-50/50">
                                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-semibold text-gray-900 sm:pl-6">Dimension {{ $dim->dimension_code }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-700 font-medium">{{ $dim->dimension_name }}</td>
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-center">
                                            <span class="inline-flex items-center rounded-md bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-700/10">Synced</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="py-12 text-center text-sm text-gray-500">
                                            No Dimensions found in local database. Use the <strong>"Sync with SAP"</strong> button to pull dimensions from Service Layer.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
