<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Withholding Taxes Master Data</h1>
                <p class="mt-1 text-sm text-gray-500">A local synchronized list of SAP Business One Withholding Tax Codes (WTax / WithholdingTaxCodes).</p>
            </div>
            <div class="flex items-center gap-3 shrink-0" x-data="{ isSyncing: false }">
                <form action="{{ route('withholding-taxes.sync') }}" method="POST" @submit="isSyncing = true">
                    @csrf
                ...
                <div x-show="isSyncing" class="fixed inset-0 z-[200] overflow-y-auto" style="display: none;">
                    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div class="relative inline-block align-middle bg-white/90 backdrop-blur-xl border border-white/40 rounded-xl text-left overflow-hidden shadow-2xl transform sm:my-8 sm:max-w-md sm:w-full p-8 text-center">
                            ...
                            <h3 class="text-xl font-semibold text-gray-900 mb-2">Syncing Withholding Taxes...</h3>
                            <p class="text-sm text-gray-500">Please wait while we fetch Withholding Tax codes from SAP Service Layer.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-8 flex flex-col sm:flex-row gap-4">
            <form method="GET" action="{{ route('withholding-taxes.index') }}" class="flex flex-1 flex-wrap gap-4 items-center">
                ...
                <div>
                    <select name="status" onchange="this.form.submit()" class="rounded-md border-0 py-2 pl-3 pr-8 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                        ...
                        <option value="inactive" {{ request('status') === 'inactive' ? 'selected' : '' }}>Inactive Taxes</option>
                ...
                @if(request()->filled('search') || request()->filled('status') || request()->has('per_page'))
                    <a href="{{ route('withholding-taxes.index') }}" class="rounded-md bg-white px-3.5 py-2 text-sm font-semibold text-red-600 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 transition-colors">Clear Filter</a>
                @endif
            </form>
        </div>
                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg bg-white">
                        <table class="min-w-full divide-y divide-gray-300">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">WHT Code</th>
                                    <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Name</th>
                                    <th scope="col" class="px-3 py-3.5 text-right text-sm font-semibold text-gray-900">Rate (%)</th>
                                    <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Category</th>
                                    <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">G/L Account</th>
                                    <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-gray-900">Status</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 bg-white">
                                @forelse($withholdingTaxes as $wt)
                                    <tr class="hover:bg-gray-50/50">
                                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-semibold text-indigo-600 font-mono sm:pl-6">{{ $wt->code }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-700 font-medium">{{ $wt->name }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-right font-mono font-semibold text-gray-900">{{ number_format($wt->rate, 2) }}%</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-600">{{ $wt->category ?? '-' }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-600 font-mono">{{ $wt->gl_account ?? '-' }}</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="py-12 text-center text-sm text-gray-500">
                                            No Withholding Taxes found matching your criteria. Use the <strong>"Sync with SAP"</strong> button to pull tax codes from Service Layer.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
